...
const storage = multer.diskStorage({
	destination: (req, file, cb) => {
		const uploadPath = path.join(__dirname, '../public/image/data');
...
/* 모든 강의 자료 최신 순으로 5개만 get (강의 이름 포함) */
    ...
    try {
        const datas = await Data.findAll({
            ...
            include: [{
                ...
                as: 'lecture',         // Data 모델에서 Lecture로 접근할 때 사용한 별칭 ('as'에 설정한 이름과 동일해야 합니다)
        ...
        console.error('자료 조회 중 오류 발생:', err); // 에러 로깅 개선
...
router.post('/write', upload.single('file'), async (req, res) => {
	...
	const imagePath = req.file ? `image/data/${req.file.filename}` : null;
	try {
