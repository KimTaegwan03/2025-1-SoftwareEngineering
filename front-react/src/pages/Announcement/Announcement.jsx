...
export default function Announcement() {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const res = await fetch(`http://localhost:3000/announcement?lecture_id=${lecture_id}`);
        const data = await res.json();
        setAnnouncements(data); // 최신순 정렬
      } catch (err) {
  ...
  const currentData = announcements.slice((page - 1) * itemsPerPage, page * itemsPerPage);
  ...
  return (
    <div className="min-h-screen bg-[#FFF8F5] px-4 py-10 font-sans">
      <div className="max-w-5xl mx-auto w-full">
        <h1 className="text-2xl sm:text-3xl font-bold mb-6 text-[#8A1601]">강의 공지사항</h1>
            ...
            <tbody>
              {currentData.map((a, idx) => (
                <tr
                  ...
                  onClick={() => navigate(`/announcement/${lecture_id}/${a.id}`)}
                >
                  <td className="py-2 px-3">
                    {announcements.length - ((page - 1) * itemsPerPage + idx)}
                  </td>
